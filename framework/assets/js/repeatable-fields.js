...
(function($) {
	$.fn.repeatable_fields = function(custom_settings) {
		var default_settings = {
			wrapper: '.vf-repeatable-wrapper',
			container: '.vf-repeatable-container',
			row: '.vf-repeatable-row',
			add: '.vf-repeatable-add',
			remove: '.vf-repeatable-remove',
			move: '.vf-repeatable-move',
			template: '.vf-repeatable-template',
			is_sortable: true,
			before_add: null,
			after_add: after_add,
			before_remove: null,
			...
			row_count_placeholder: 'vf-repeatable-input-placeholder',
		}
		...
		function after_add(container, new_row) {
			...
			$('*', new_row).each(function() {
				$.each(this.attributes, function(index, element) {
					this.value = this.value.replace(settings.row_count_placeholder, []);
				});
			});
